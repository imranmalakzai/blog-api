...
export const createArticle = async (data) => {
  const [result] = await pool.query(
    `INSERT INTO articles 
     (author_id, title, slug, excerpt, content, status, visibility, published_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      data.author_id,
      data.title,
...
export const getAPublicationArticleById = async (publicationId, articleId) => {
  const [rows] = await pool.query(
    "SELECT * FROM articles WHERE publication_id = ? AND id = ? AND deleted_at IS NULL",
...
export const myArticles = async (userId) => {
  const [rows] = await pool.query(
    "SELECT * FROM articles WHERE status = 'published' AND visibility = 'public' AND deleted_at IS NULL AND author_id = ? ",
    [userId]
  );
  return rows;
};
