...
export const createPublicationMember = async (data) => {
  const [result] = await pool.query(
    "INSERT INTO publication_members (publication_id,user_id)  VALUES (?,?)",
    [data.publication_id, data.user_id]
  );
  result.insertId;
};
...
export const publicationMemebers = async (publicationId) => {
  const [rows] = await pool.query(
    "SELECT u.id as userId,u.username,u.avatar_url FROM publication_members pm JOIN  users u ON u.id = pm.user_id  WHERE publication_id = ?",
    publicationId
  );
  return rows;
};
...
export const isPublicationMemeber = async (publicationId, userId) => {
  const [rows] = await pool.query(
    `SELECT * FROM publication_memebers WHERE publication_id = ? AND user_id = ?`,
