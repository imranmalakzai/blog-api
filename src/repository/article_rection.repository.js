...
export const usersLikedArticle = async (articleId) => {
  ...
  const [rows] = await pool.query(
    "SELECT r.name as reaction,u.id as userId,u.username,u.avatar_url from article_reactions a JOIN users u ON a.user_id = u.id JOIN reactions r ON a.reaction_id = r.id  where a.article_id = ?",
    [articleId],
  );
  return rows;
};
